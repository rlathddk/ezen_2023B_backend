...
public class Step1 {
    public static void main(String[] args) {
        ...
        int[] intArray = null;
            // 스택영역에 변수 생성 : 'intArray' 지역 변수 선언
//        intArray[0] = 10; //.NullPointerException
            // 배열변수는 있지만. 배열객체(힙영역)가 실제로 존재하지 않음.
        String str = null;
            // 스택영역에 변수 생성 : 'str'
/*
...
*/
        ...
        // GC = 쓰레기 수집기
            ...
            //      - 그 객체의 모든 참조를 없애는 방법을 쓴다 -> null
        String hobby = "여행";
            // String hobby : 스택영역에 생성된 변수
            // "여행" : 힙영역에 생성된 문자열객체 [102번지](예시임)
                // = 대입 102번지를 hobby변수에 대입
                // !! : hobby변수에는 "여행"문자열을 가지고 있지 않다. (주소를 갖고있는 거지 문자를 가지고 있는 거 x)
        hobby = null;
                // hobby변수가 null를 가짐
                // "여행" 객체는 현재 아무도 참조하지 않기 떄문에 잠시후 자동으로 힙영역 삭제
        String kind1 = "자동차";
        // String kind1 : 스택영역          힙 "자동차"[103번지]
        String kind2 = kind1;
        // String kind1 : 스택영역          kind1[103번지]
        kind1 = null;
            // "자동차" 객체는 사라지지 않는다. 이유는 kind2변수가 참조중이므로
        ...
        /*
            ...
            4. 항목/요소 삭제 : 없음 [자바는 힙영역을 제거하는 방법을 제공하지 않는다. GC]
                - 배열의 크기는 줄이거나 늘릴 수 없다. -> 방법 : 1. 직업 함수 만들기, 2. 라이브러리(컬렉션프레임워크)
            5. 배열의 구조
            ...
            스택영역                        힙영역
                String[] array1             1. 배열객체 = {103번지(유재석) . 104번지(강호동)};   100번지
                                            2. 문자열(유재석)객체 =              103번지
                                            3. 문자열(강호동)객체 =              104번지
            -------------------------------------------------------------------------------------------
                String[] array2 = new String[2] // 자리만 미리 만들어 놓고
            스택영역                        힙영역 new String[2]
                String[] array2             1. 배열객체 = {null, null};         201번지
                                            2. 배열객체 = {204번지, null}        201번지
                1. 배열내 특정 항목 대입
                array2[0] = "유재석"
                                            2. 문자열(유재석)객체 =               204번지  // 주소안에 주소 ex. 101동안에 102호

         */

        // 1. 배열의 선언
        String[] array1 = {"유재석", "강호동"}; // 초기값 먼저 선언
    ...
    // main()함수는 자바 진입/시작점 --> main함수 끝나면 힙영역은 모두 사라짐
