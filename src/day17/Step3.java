...
public class Step3 {
    ...
    2. [인터페이스]Connection
        - stetement, preparedStatement 구현 객체를 반환한다.
        1. connection, prepareStatement(SQL);   SQL서명/기재된 Statement 구현체 반환한다
    ...
    4. [인터페이스]ResultSet
        ...
        2. 현재 레코드에서 필드 값 호출 : .get타입(호출할 필드 순서번호or필드명);
            ...
            try {
                ...
                if( ch== 1){
                   ...
                   while (resultSet.next()){ // 검색결과내 첫번째 레코드부터 마지막레코드까지 순회
                       // 하나씩 next()로 이동하면서 반복처리 만약에 다음레코드가 존재하면 실행.
                    ...
                    // 1. 입력받기
                    System.out.println("수정할 고객명 : "); String oldName = scanner.next(); // 누구를
                    System.out.println("새로운 고객명 : "); String newName = scanner.next(); // 어떻게
                    // 2. DB에 수정처리 한다.
                    // String sql = "update memebers set name = '새로운고객명' where name = '수정할대상'";
                    ...
                    // 1. 입력받기 // 누구를 삭제할껀지 입력
                    System.out.println("삭제할 고객명 : ");
                    String deleteName = scanner.next();
                        // 2. db에 삭제처리한다
                    // String sql = "delete from memebers"; // 전체삭제 됨
                    // String sql = "delete from memebers where name = '삭제할고객명'";
                ...
                System.out.println("<시스템오류>SQL에서 오류가 발생하였습니다.");
                System.out.println(e);
            }
        } // w e
    } // m e
